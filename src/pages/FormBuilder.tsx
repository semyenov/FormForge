  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-6 card">
        <div className="mb-4">
          <label htmlFor="formName" className="block mb-1 text-sm font-medium text-gray-700">
            Form Name
          </label>
          <input type="text" id="formName" value={formName} onChange={(e) => setFormName(e.target.value)} className="form-input" placeholder="Enter form name" />
        </div>
        ...
        <div>
          ...
          <textarea id="formDescription" value={formDescription} onChange={(e) => setFormDescription(e.target.value)} className="form-input" rows={3} placeholder="Enter form description" />
        </div>
      </div>
        ...
        <div key={field.id} className="mb-4 border-l-4 border-blue-500 card">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="flex space-x-2">
              <button onClick={() => moveField(field.id, "up")} disabled={index === 0} className="p-1 text-gray-500 hover:text-gray-700 disabled:opacity-30">
                <MoveUp className="w-5 h-5" />
              </button>
              <button onClick={() => moveField(field.id, "down")} disabled={index === fields.length - 1} className="p-1 text-gray-500 hover:text-gray-700 disabled:opacity-30">
                <MoveDown className="w-5 h-5" />
              </button>
              <button onClick={() => removeField(field.id)} className="p-1 text-red-500 hover:text-red-700">
          ...
          <div className="mb-4">
            <label className="block mb-1 text-sm font-medium text-gray-700">Question / Label</label>
            <input type="text" value={field.name} onChange={(e) => updateField(field.id, { name: e.target.value })} className="form-input" placeholder="Enter question or label" />
          </div>

          <div className="mb-4">
            <label className="block mb-1 text-sm font-medium text-gray-700">Field Type</label>
            <select value={field.type} onChange={(e) => updateField(field.id, { type: e.target.value as FormTemplateField["type"] })} className="form-input">
          ...
          <div className="flex items-center">
            <input type="checkbox" id={`required-${field.id}`} checked={field.required} onChange={(e) => updateField(field.id, { required: e.target.checked })} className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500" />
      ...
      {isAddingField ? (
        <div className="mb-6 card">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            {fieldTypeOptions.map((option) => (
              <button key={option.value} onClick={() => addField(option.value as FormTemplateField["type"])} className="flex flex-col justify-center items-center p-4 rounded-lg border border-gray-200 transition-colors hover:bg-gray-50">
          ...
          <div className="flex justify-end mt-4">
            <button onClick={() => setIsAddingField(false)} className="btn btn-secondary">
              Cancel
            </button>
          </div>
        </div>
      ) : (
        <button onClick={() => setIsAddingField(true)} className="flex justify-center items-center py-3 w-full text-gray-500 rounded-lg border-2 border-gray-300 border-dashed transition-colors hover:border-blue-500 hover:text-blue-500">
